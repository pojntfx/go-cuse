...
func ReplyIoctl(req Request, result int, buf unsafe.Pointer, size Size) error {
	if ret := C.fuse_reply_ioctl(req, C.int(result), buf, C.ulong(size)); ret != 0 {
		return fmt.Errorf("could not reply with ioctl: error code %v", ret)
	}
